...
var number;
  ...
  function animate(){
   ...
   for(i=1; i<3; i++){ document.image_rotate.src = myimages[number];
    ...
    <form id="action-form" action=?<?php htmlout($action); ?> method="post" >
	 ...
	 <label for="term">Payment Type:</label>
        <select name="ptype"  id="irmsnputState" class="form-control md-3">
          <option value="">Select one</option>
          <?php foreach ($ptypes as $ptype): ?>
            <option value="<?php htmlout($ptype['id']); ?>"<?php
               if ($ptype['selected'])
              {
                echo 'selected';
              }
                ?>>
                    <?php htmlout($ptype['name'].' - '.number_format($ptype['amount'],2)); ?></option>
          <?php endforeach; ?>
        </select>
      </div>
	   <div class="col-md-6 mb-6">
      <label for="validationDefault07">Amount:</label>
      <input type="text" name="amount" class="form-control" id="validationDefault07" value="<?php if (isset($amount)) htmlout($amount); else echo ''; ?>" placeholder="Amount" required>
    </div>
  ...
  <div class="row">
    ...
    <label for="session">Term:</label>
        <select name="terms"  id="inputState" class="form-control md-3">
          <option value="">Select one</option>
